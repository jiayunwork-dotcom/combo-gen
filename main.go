...
// argument may safely appear before flags (go's flag stops at the first positional).
...
// splitList turns "a, b ,c" into ["a" "b" "c"], dropping blank entries.
...
func main() {
	if len(os.Args) < 2 {
		fatal("usage: combo-gen <perm|comb|product> [args] [flags]")
	}
	cmd := os.Args[1]
	...
	case "product":
		runProduct(args)
	default:
		fatal("unknown command %q (want perm|comb|product)", cmd)
	}
}
