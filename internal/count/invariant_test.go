...
// These tests verify the fundamental invariant: the count formula must
// equal the number of results from the enumeration.
...
func TestInvariantCombinationsLexOrder(t *testing.T) {
	items := []string{"a", "b", "c", "d"}
	got, _ := comb.Combinations(items, 2)
	// each combination should have elements in input order
	for _, combo := range got {
		for i := 1; i < len(combo); i++ {
			// find positions in items
			pos0, pos1 := -1, -1
